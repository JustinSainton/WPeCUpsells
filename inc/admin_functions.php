...
function wpsc_generate_pdf_sales_log( $purchase_id, $file_format = 'D' ) {

	$html = wp_remote_retrieve_body( wp_remote_get( add_query_arg( array( 'pdf_handler' => 'true', 'purchase_id' => $purchase_id, 'ecse_type' => 'receipt' ), home_url() ) ) );
	...
	$file_name = 'purchase-number-' . $purchase_id . '.pdf';
	$file_name = $file_format == 'D' ? $file_name : '/home/shirmich/public_html/wp-content/' . $file_name;
	$mpdf->Output( $file_name, $file_format );
...
if ( isset( $_GET['purchase_id'] ) && isset( $_GET['pdf_handler'] ) )
  add_action( 'init', 'wpsc_generate_pdf', 15 );

function wpsc_generate_pdf() {
  ecse_get_template_preview_on_own_page();
}
